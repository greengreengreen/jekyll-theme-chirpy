...
date: 2021-02-27 11:15:00 
categories: [Data Stuctures And Algorithms]
...
Ceiling means: m = l + math.ceil((r-l)/2). floor means: m = l + (r-l)//2 . <br>
...
if we use ceiling, then whenever r-l is odd, m will be (r-l)/2 + 0.5. <br>
For example, when l = 0, r = 1, ceiling will always return m = 1. In this case, we will never be able to test whether or not arr[0] qualifies. And arr[0] could be the first qualified index. <br>
Therefore, if we use ceiling method to get the first qualified index, we might never be able to catch and fall into infinite loop. <br>
